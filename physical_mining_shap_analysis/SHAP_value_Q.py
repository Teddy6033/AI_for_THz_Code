...
pbz2_path = r'..\data_space\dataset_pbz2\shap_values_Q.pbz2'
...
# 保存为高分辨率图像
# plt.savefig(r'..\data_space\shap_figures\shap dot on Q.png', dpi=1200)  # 300 DPI for high resolution
plt.show()
shap.summary_plot(shap_values, X_test, plot_type="bar", feature_names=features)
# 获取当前 Axes 对象
# ax = plt.gca()
...
# # plt.title("Q",fontsize=15)
# # 保存为高分辨率图像
# plt.savefig('E:\桌面\可解释性图表\高分辨\shap bar on Q.png', dpi=1200)  # 300 DPI for high resolution
# plt.show()
...
plt.ylabel('Importance to Q')
plt.title('DL:SHAP to Q')
plt.show()
