...
title = "spectrum-loaded-prediction-network"
...
class MainForwardNet(object):
    def __init__(self, args):
        ...
        self.set_seed()
        # 准备数据,创建数据加载器
        dataset_tar = torch.load(r"../data_space/datasets_tar/loaded_dataset_forward_for_technology.pth")
    ...
    def train(self):
        ...
        for epoch in range(epoch + 1, self.args.epochs + 1):
            ...
            for index, (structure_nor, valley_pos, spectrum_total, structure_raw) in enumerate(self.train_loader):
                structure = structure_nor.to(self.args.device)
                spectrum_total = spectrum_total.to(self.args.device)

                # 正向传播
                spectrum_out = model(structure)
                # 计算损失
                loss = criterion(spectrum_total, spectrum_out)
                # 将梯度清零
                optimizer.zero_grad()
                # 反向传播
                loss.backward()
    ...
    def validation(self, model):
        ...
        with torch.no_grad():
            ...
            for index, (structure_nor, valley_pos, spectrum_total, structure_raw) in enumerate(self.val_loader):
                structure = structure_nor.to(self.args.device)
                spectrum_total = spectrum_total.to(self.args.device)
                ...
                spectrum_out = model(structure)
                # 计算损失
                running_loss += self.criterion(spectrum_total, spectrum_out)
