...
from forward_design_network import ForwardNetMlp as F_Net

title = "bidirectional-forward-design-network-test"
    ...
    def train(self):
        ...
        for epoch in range(epoch + 1, self.args.epochs + 1):
            ...
            for index, (structure_nor, valley_pos, spectrum_total, structure_raw) in enumerate(self.train_loader):
                ...
                # 计算损失
                loss = criterion(predict_valley, valley_pos)
                # 将梯度清零
                optimizer.zero_grad()
                # 反向传播
                loss.backward()
    ...
    def validation(self, model):
        ...
        with torch.no_grad():
            ...
            for index, (structure_nor, valley_pos, spectrum_total, structure_raw) in enumerate(self.val_loader):
                ...
                predict_valley = model(structure)
                # 计算损失
                running_loss += self.criterion(predict_valley, valley_pos)
