...
forward_tar = r"../data_space/checkpoints/feature-W-prediction-network-checkpoint.pth.tar"
...
def evaluate_model(loader, model, device):
    ...
    with torch.no_grad():
        for index, (structure_nor, F, Q, I, W) in enumerate(loader):
            ...
            # 保存预测值和标签
            all_preds.append(predict_W.cpu().numpy())
            all_labels.append(W.cpu().numpy())

            # 计算损失并累加
            loss = nn.MSELoss()(predict_W, W)  # 直接计算MSE
...
print(f"\nTrain MSE: {train_loss:.7f}   Validation MSE: {val_loss:.7f}")  # MSE
