...
def evaluate_model(loader, model, device):
    ...
    with torch.no_grad():
        for index, (structure_nor, F, Q, I, W) in enumerate(loader):
            structure = structure_nor.to(device)
            F = F.to(device)

            # 正向传播
            predict_F = model(structure)

            # 保存预测值和标签
            all_preds.append(predict_F.cpu().numpy())
            all_labels.append(F.cpu().numpy())

            # 计算损失并累加
            loss = nn.MSELoss()(predict_F,F)  # 直接计算MSE
...
print(f"\nTrain MSE: {train_loss:.8f}   Validation MSE: {val_loss:.7f}")  # MSE
print(f"Train RMSE: {train_rmse:.7f}   Validation RMSE: {val_rmse:.7f}")  # RMSE
print(f"Train R²: {train_r2:.7f}   Validation R²: {val_r2:.7f}")      # R²
print(f"Train MAE: {train_mae:.7f}   Validation MAE: {val_mae:.7f}")    # MAE
