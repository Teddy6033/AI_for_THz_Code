...
class DataSet(Dataset):
        ...
        for file_name in self.file_list:  # random.sample(files, nums):
            if file_name.endswith(".txt"):
                ...
                if (device_raw.structure_nor.__len__() + device_raw.spectrum_total.__len__() +
                    ...
                    # cross_entropy()中target要求是long型
                    # device.valley_index = torch.Tensor([device_raw.valley_index]).to(torch.long)
                    device.valley_pos = torch.Tensor([device_raw.valley_index * 0.001])
    ...
    def __getitem__(self, index):
        ...
        return device.structure_nor, device.valley_pos, device.spectrum_total, device.structure_raw
...
if __name__ == "__main__":
    ...
    torch.save(state_info, r"..\data_space\datasets_tar\unloaded_dataset_forward_for_technology_test.pth")
